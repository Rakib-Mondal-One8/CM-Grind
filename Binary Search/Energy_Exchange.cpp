...
void solve()
{
	...
	lld l = 0, r = 5 * 1e3, error = 1e-7;
	for (int i = 0; i < 40; i++) {
		lld mid = l + (r - l) / 2.00;
		lld need = 0 , inventory = 0;
		for (int i = 0; i < n; i++) {
			if (a[i] >= mid)inventory += ((a[i] - mid) * (100 - k)) / 100.00; // x-(x*k)/100;
			else need += mid - a[i];
		}
		if (inventory >= need)l = mid + error;
		else r = mid - error;
	}
	cout << setprecision(7) << fixed << r << nl;
}
int main()
{
	init_code();
