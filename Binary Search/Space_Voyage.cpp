...
/*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
ll check(vector<ll>&a, vector<ll>&b, ll n, ll x) {
	ll days = 0;
	loop(i, 0, n - 1) {
		days += 1;
		days += ((a[i] * x) / b[i]);
...
void solve()
{
	...
	auto go1 = [&]() {
		...
		while (l <= r) {
			ll mid = l + (r - l) / 2;
			ll get = check(a, b, n, mid);
			if (get > c) {
				r = mid - 1;
			}
			else if (get < c)l = mid + 1;
	...
	debug(left);
	debug(right);
	if (left != -1 && right != -1)cout << right - left + (left != 0 && right != 0) ? 1 : 0 << nl;
	else if (left != -1)cout << 1 << nl;
	else if (right != -1)cout << 1 << nl;
	else cout << -1 << nl;
}
int main()
{
