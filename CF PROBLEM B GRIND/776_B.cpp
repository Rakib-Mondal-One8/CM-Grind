...
/*Problem Link -> https://codeforces.com/problemset/problem/1650/C*/
...
void solve()
{
	...
	ll tot = 0;
	for (int i = 0; i < n; i++) {
		tot += v[i].first.first + v[2 * n - i - 1].first.first;
		ans.push_back({v[i].first.second, v[i].second});
		ans.push_back({v[2 * n - i - 1].first.second, v[2 * n - i - 1].second});
	}
	...
	n = ans.size();
	for (int i = 0; i < n / 2; i++) {
		cout << ans[i].second + 1 << " " << ans[n - i - 1].second + 1 << nl;
	}
	cout << nl;
}
int main()
