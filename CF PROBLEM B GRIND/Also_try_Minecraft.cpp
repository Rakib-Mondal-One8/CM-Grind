...
void solve()
{
	...
	pref[1] = 0;
	loop(i, 2, n) {
		if (a[i - 1] - a[i] > 0)pref[i] = pref[i - 1] + (a[i - 1] - a[i]);
		else pref[i] = pref[i - 1];
	}
	suff[n] = 0;
	for (int i = n - 1; i >= 1; i--) {
		if (a[i + 1] - a[i] > 0)suff[i] = suff[i + 1] + (a[i + 1] - a[i]);
		else suff[i] = suff[i + 1];
	}
	for (auto e : b) {
		if (e.first > e.second) {
			cout << suff[e.second] - suff[e.first] << nl;
		}
		else cout << pref[e.second] - pref[e.first] << nl;
	}
}
int main()
{
